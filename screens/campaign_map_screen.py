...
# ----------------- UI ส่วนจัดการทหาร (Recruit / Manage) -----------------
...
class MarchSetupPopup(ModalView):
    def __init__(self, source_node, target_node, app, map_screen, **kwargs):
        ...
        btn_str = "[b]ENGAGE COMBAT[/b]" if target_node.faction != source_node.faction else "[b]MERGE ARMY[/b]"
        ...
        btn_box.add_widget(btn_cancel)
        btn_box.add_widget(self.btn_confirm)
        root.add_widget(btn_box)

        self.add_widget(root)
        self.build_transfer_ui()
    ...
    def build_transfer_ui(self):
        ...
        for p in ['king', 'prince', 'queen', 'rook', 'bishop', 'knight', 'pawn']:
            if self.temp_base_army.get(p, 0) > 0: 
                ...
                counter = TroopTransferCounter(p, self.temp_base_army[p], self.on_transfer_attempt, img_path)
                self.grid.add_widget(counter)
    ...
    def on_transfer_attempt(self, piece_name, direction):
        if direction == 1: 
            ...
            self.marching_army[piece_name] += 1
            self.temp_base_army[piece_name] -= 1
        else: 
            self.marching_army[piece_name] -= 1
            self.temp_base_army[piece_name] += 1
    ...
    def update_status(self):
        ...
        total = headers + heavies + lights
        max_cap = 18 if headers > 0 else 8
        self.lbl_status.text = f"Marching Capacity: [b]{total} / {max_cap}[/b]  |  [color=ffcc00]Header: {'Yes' if headers > 0 else 'No'}[/color]"

    def confirm_march(self, instance):
        total_marching = sum(self.marching_army.values())
        if total_marching == 0: return 
        
        self.app.play_click_sound()
        self.source_node.army = self.temp_base_army.copy() 
        
        if self.target_node.faction == self.source_node.faction:
            for p, count in self.marching_army.items():
                self.target_node.army[p] += count
            print(f"Merged army into {self.target_node.node_id}")
        else:
...
class NodeActionMenu(ModalView):
    def __init__(self, node, app, map_screen, **kwargs):
        ...
        btn_manage = Button(text="[b]MANAGE ARMY[/b]\n(Recruit & Upgrade)", markup=True, background_color=(0.2, 0.4, 0.8, 1))
        btn_manage.bind(on_release=self.open_manage)
        root.add_widget(btn_manage)

        btn_march = Button(text="[b]MARCH / ATTACK[/b]\n(Move to connected base)", markup=True, background_color=(0.8, 0.2, 0.2, 1))
    ...
    def open_manage(self, instance):
        self.app.play_click_sound()
        self.dismiss()
        ArmyManagementPopup(node=self.node, app=self.app).open()

    def start_march(self, instance):
        self.app.play_click_sound()
        self.dismiss()
        self.map_screen.start_marching(self.node)
...
class MapNode(Button):
    def __init__(self, node_type, faction, node_id, is_main_base=False, **kwargs):
        ...
        self.has_claimed_prince = False # สำหรับตอนที่ตีปราสาท 2 หรือ 3 แตก
        self.neighbors = []             

        # ✨ กองทัพเริ่มต้น
        if self.is_main_base:
            # ปราสาทหลัก: ไม่มี Prince แต่อย่างใด จนกว่าจะไปตีปราสาทที่ 2-3
            self.army = {'king': 1, 'queen': 1, 'rook': 2, 'bishop': 2, 'knight': 2, 'pawn': 8, 'prince': 0}
        elif faction == 'red':
            # กองทัพของ Red (Neutral) เริ่มมามีเยอะ แต่จะโดนสุ่มลดลงตอนเข้าตี (8-12 ตัว)
            if node_type == 'castle':
                self.army = {'king': 1, 'queen': 1, 'rook': 2, 'bishop': 2, 'knight': 2, 'pawn': 8, 'prince': 0}
            else:
                self.army = {'king': 0, 'queen': 0, 'rook': 1, 'bishop': 1, 'knight': 1, 'pawn': 8, 'prince': 0}
        else:
    ...
    def on_release(self):
        ...
        if self.faction == current_turn:
            NodeActionMenu(node=self, app=app, map_screen=map_screen).open()
        else:
...
class CampaignMapScreen(Screen):
    ...
    def initiate_combat(self, source_node, target_node):
        print(f"STATE 2: COMBAT INITIATED! {source_node.node_id} -> {target_node.node_id}")
        app = App.get_running_app()
        
        app.combat_source = source_node
        app.combat_target = target_node
        
        # ✨ สุ่มกองทัพศัตรูสีแดง (Red Neutral Army) ให้เหลือ 8-12 ตัวแบบสุ่ม
        ...
        if target_node.faction == 'red':
            ...
            # เลือกตัวที่จะตัดออกได้ (ไม่ตัดหัวหน้า King, Prince)
            ...
            print(f"Red Army randomized down to {current_count} units.")
